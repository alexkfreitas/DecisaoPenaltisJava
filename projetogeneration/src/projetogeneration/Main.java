...
public class Main {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Boolean penaltisAlternados = true;
		...
		for (int i = 1; i <= 5; i++) {
			...
			if(primeiro.getGol() - segundo.getGol() == 3 || segundo.getGol() - primeiro.getGol() == 3) {
		...
		if(penaltisAlternados && primeiro.getGol() == segundo.getGol()) {
			System.out.println("\nA decisão vai para os pênaltis alternados!!");
		do {
			if (player == 1) {
				//TimeUnit.SECONDS.sleep(2);
				System.out.println("|");
		...
		do {
			...
			System.out.println("FINAL DA SÉRIE DE 5 COBRANÇAS!! TUDO IGUAL ATÉ AQUI! VAMOS PARA AS COBRANÇAS ALTERNADAS");
			//TimeUnit.SECONDS.sleep(2);
